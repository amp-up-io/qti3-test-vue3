...
export class CatalogFactory {
  ...
  constructor (test, store) {
    this.test = test
    this.testElement = test.$refs.test
    this.store = store
  ...
  bindAll () {
    ...
    // Get catalog-idref's
    this.nodeList = this.testElement.querySelectorAll('[data-catalog-idref]')
  ...
  bindGlossary (nodes) {
    ...
    const pnpGlossarySupports = this.getPnpGlossarySupports(this.store.getTestContextPnp())
  ...
  showGlossaryHandler (event) {
    ...
    this.test.$parent.$emit('testCatalogEvent', {
      type: 'glossary',
      catalogIdRef: idref,
