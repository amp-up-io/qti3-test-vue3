...
export default class QtiAttributeValidation {
  ...
  QTI_SUBMISSION_MODES () {
    // enumerated vocabulary of submission-mode attribute
    return [
      'individual',
      'simultaneous'
    ]
  ...
  QTI_NAVIGATION_MODES () {
    // enumerated vocabulary of navigation-mode attribute
    return [
      'linear',
      'nonlinear'
    ]
  ...
  validateBooleanAttribute (attributeName, attributeValue, isRequired, defaultValue) {
    if (typeof attributeValue === 'undefined') {
      if (isRequired) {
        throw new QtiValidationException('Attribute "' + attributeName + '" is a required attribute')
      } else {
  ...
  validateSubmissionModeAttribute (mode) {
    if (this.QTI_SUBMISSION_MODES().includes(mode)) {
      return true
    }
    throw new QtiValidationException('Unsupported "submission-mode" attribute: "' + mode + '".')
  }

  validateNavigationModeAttribute (mode) {
    if (this.QTI_NAVIGATION_MODES().includes(mode)) {
      return true
    }
    throw new QtiValidationException('Unsupported "navigation-mode" attribute: "' + mode + '".')
  }
