...
export default {
  ...
  methods: {
    ...
    validateAttributes() {
      let declaration
      try {
        declaration =  qtiAttributeValidation.validateVariableIdentifierAttribute(teststore, this.pattern)
    ...
    getRegex () {
      if (this.isPatternIdentifier) {
        let declaration = teststore.getVariableDeclaration(this.pattern)
