...
<script>
/*
 * This provides the 'ElseIf' clause of the 'If..Then..Else' for the outcome
 * processing functionality. An outcomeElseIf part consists of an expression which must have an
 ...
 * (including if the expression is NULL) and the following outcomeElseIf or outcomeElse
 * parts (if any) are considered instead.
 *
 * qti-outcome-else-if is defined in an identical way to qti-outcome-if.
 */
    ...
    validateChildren () {
      ...
      this.$slots.default.forEach((slot) => {
        if (qtiAttributeValidation.isValidSlot(slot)) {
          ...
          else if (qtiProcessing.isOutcomeRuleNode(slot.componentOptions.tag)) {
            if (countExpression === 0) {
              throw new QtiValidationException('Outcome Rules must follow an Expression node: "' + slot.componentOptions.tag + '"')
            }
          } else {
            throw new QtiValidationException('Node is not an Expression or an Outcome Rule: "' + slot.componentOptions.tag + '"')
          }
        }
      })
      if (countExpression === 0) {
